import { Locale, addDays, addMinutes, addMonths, differenceInMinutes, format, isAfter, isPast, isSameDay, isSameMinute, isToday, subDays, subMonths } from "date-fns";
...
export const ScheduleMeeting: React.FC<Props> = ({
  ...
  const calendarColoredTextRGB = isBackgroundColorDark ? Color(primaryColor).lighten(0.5).rgb().array().join(",") : Color(primaryColor).darken(0.5).rgb().array().join(",");

  const [selectedStartTime, setSelectedStartTime] = useState<number | undefined>(_selectedStartTime ? _selectedStartTime.getTime() : undefined);
  ...
  useEffect(() => {
    ...
    const _nextFutureStartTimeAvailable = startTimeEventsList.find((startTime) => isAfter(startTime.startTime, selectedDay) && !isToday(startTime.startTime))?.startTime;

    if (startTimeEventsList.length > 0 && onNoFutureTimesAvailable && !_nextFutureStartTimeAvailable && orderedEvents.length === 0) {
      ...
      <Inner>
        <CalendarContainer>
          <Header>
            ...
            <SelectedDayTitle className="rsm-date-title">{format(selectedDay, format_selectedDateMonthTitleFormatString, { locale })}</SelectedDayTitle>
            <ArrowButton type="button" className="rsm-arrow-button" onClick={goToNextMonth}>
              <Arrow direction="forward" />
            </ArrowButton>
          </Header>
          <ScheduleCalendar locale={locale} selectedDay={selectedDay} availableTimeslots={orderedAvailableTimeslots} onDaySelected={onDaySelected} />
        </CalendarContainer>
        <Divider />
        <StartTimeListContainer>
          <StartTimeListContainerAbsolute>
            <Header>
              ...
              <SelectedDayTitle className="rsm-date-title">{format(selectedDay, format_selectedDateDayTitleFormatString, { locale })}</SelectedDayTitle>
