  ...
  const emptyListElement = (
    <NoTimesAvailableContainer>
      <>
        ...
        {nextFutureStartTimeAvailable ? (
          <GoToNextAvailableDayButton type="button" selected className="rsm-next-available-date-button" onClick={onGoToNextAvailableDayClick}>
            <p>
          ...
          <ScrollListContainer>
            {startTimeListItems.map((startTimeEvent: any, i: number) => (
              <React.Fragment key={i}>
                ...
                />
                {i !== startTimeListItems.length - 1 && <ListItemDivider makeTransparent={selectedItemIndex === i || selectedItemIndex === i + 1} />}
              </React.Fragment>
            ))}
          </ScrollListContainer>
        </>
        ...
        <GridContainer className={selectedStartTime ? "has-selection" : ""}>
          {startTimeListItems.map((startTimeEvent: StartTimeEvent, i: number) => (
            <StartTimeGridItemButton
              key={i}
              type="button"
              className={selectedStartTime && selectedStartTime === startTimeEvent.startTime.getTime() ? "is-selected" : ""}
